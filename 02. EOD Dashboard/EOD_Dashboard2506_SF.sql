...
-- Added in_clearwater in oceanside on 27-05
-- same process need to follow for uncommented part, if used
-- Press No changed on 28-05
-- Operations data on 07-02
-- Testing on 03-07
-- Sioux Falls- LA on 04-07
--


/*	TRUNCATE table dt_inovar_prod_edw.u_mat_sales_bookings_eod_dashboard

	
INSERT INTO dt_inovar_dev_stg.u_mat_sales_bookings_eod_dashboard_test
SELECT * FROM dt_inovar_dev_stg.v_sales_bookings_eod_dashboard_test	


		
CREATE TABLE dt_inovar_dev_stg.u_mat_sales_bookings_eod_dashboard_test AS 
SELECT * FROM dt_inovar_dev_stg.v_sales_bookings_eod_dashboard_test
...
select count(*),sum(backlog_amount) backlog_amount,
...
/*drop view dt_inovar_dev_stg.v_sales_bookings_eod_dashboard_test


drop table dt_inovar_dev_stg.u_mat_sales_bookings_eod_dashboard_test




CREATE OR REPLACE VIEW dt_inovar_dev_stg.v_sales_bookings_eod_dashboard_test as*/


select distinct "Business_Unit" from dt_inovar_prod_stg.in_press_description pd
...
select distinct entity from press
...
select
	...
	0::float																		as footused_tabco_ex_booklet,
	0::float																		as footused_tabco_good_ex_booklet
from dt_inovar_prod_stg.in_clearwater_fact_kpi_work_order_by_associate_by_date pbd		------
...
select
	...
	0::float																		as footused_tabco_ex_booklet,
	0::float																		as footused_tabco_good_ex_booklet
from dt_inovar_prod_stg.in_clearwater_fact_kpi_downtime_hours_by_date pbd
...
where pressno is not null and pressno <> ''
	union all
select 'Milwaukee' 										AS businessunit,
	...
	0::float											as footused_tabco_ex_booklet,
	0::float											as footused_tabco_good_ex_booklet 
from (select *, /*case when entity = 'Cimarron North' and workoperation ~ '^\d' then right(workoperation, (length(workoperation) - 4)) else workoperation end*/ workoperation as workoperation_clean 
		from dt_inovar_prod_stg.in_butler_fact_timecard) tc
left join press p
	on trim(lower(p.press)) = trim(lower(tc.pressno)) and p.entity = 'Butler'
left join  dt_inovar_prod_stg.in_butler_fact_ticket  tkt 
	on tkt.number = tc.ticket_no /*and tkt.entity = tc.entity*/
left join press pt 
	on trim(lower(pt.press)) = trim(lower(tkt.press)) and pt.entity = 'Butler'
...
union all	
	...
	0::float											as footused_tabco_good_ex_booklet
from (select *,/*case when entity = 'Cimarron North' and workoperation ~ '^\d' then right(workoperation, (length(workoperation) - 4)) else workoperation end*/ workoperation  as workoperation_clean 
		from dt_inovar_prod_stg.in_dallas_fact_timecard ) tc
left join press p
	on trim(lower(p.press)) = trim(lower(tc.pressno)) and p.entity = 'Dallas'
left join  dt_inovar_prod_stg.in_dallas_fact_ticket tkt 
	on tkt.number = tc.ticket_no /*and tkt.entity = tc.entity*/
left join press pt 
	on trim(lower(pt.press)) = trim(lower(tkt.press)) and pt.entity = 'Dallas'
where tc.sdate >= '2015-01-01'
	and tc.sdate <= current_date
	and tc.workoperation not in ('Punch In', 'Punch Out')
	and tc.pressno is not null and tc.pressno <> ''
union all 
	select 'Ft. Lauderdale'								AS businessunit,
	tc.workoperation_clean								as workoperation,
	tc.sdate,
	case
		when tc.edate = '' then EXTRACT(EPOCH FROM tc.elapsed::time)
	...
	0::float											as footused_tabco_good_ex_booklet
from (select *,/*case when entity = 'Cimarron North' and workoperation ~ '^\d' then right(workoperation, (length(workoperation) - 4)) else workoperation end*/ workoperation as  workoperation_clean 
		from dt_inovar_prod_stg.in_davie_fact_timecard ) tc
left join press p
	on trim(lower(p.press)) = trim(lower(tc.pressno)) and p.entity = 'Davie'
left join  dt_inovar_prod_stg.in_davie_fact_ticket  tkt 
	on tkt.number = tc.ticket_no /*and tkt.entity = tc.entity*/
left join press pt 
	on trim(lower(pt.press)) = trim(lower(tkt.press)) and pt.entity = 'Davie'
where tc.sdate >= '2015-01-01'
	and tc.sdate <= current_date
	and tc.workoperation not in ('Punch In', 'Punch Out')
	and tc.pressno is not null and tc.pressno <> ''
union all 
	select 'Newburyport'								AS businessunit,
	tc.workoperation_clean								as workoperation,
	tc.sdate,
	case
		when tc.edate = '' then EXTRACT(EPOCH FROM tc.elapsed::time)
	...
	0::float											as footused_tabco_good_ex_booklet
from (select *,/*case when entity = 'Cimarron North' and workoperation ~ '^\d' then right(workoperation, (length(workoperation) - 4)) else workoperation end*/ workoperation as  workoperation_clean 
		from dt_inovar_prod_stg.in_newburyport_fact_timecard ) tc
left join press p
	on trim(lower(p.press)) = trim(lower(tc.pressno)) and p.entity = 'NewBuryPort'
left join  dt_inovar_prod_stg.in_newburyport_fact_ticket  tkt 
	on tkt.number = tc.ticket_no /*and tkt.entity = tc.entity*/
left join press pt 
	on trim(lower(pt.press)) = trim(lower(tkt.press)) and pt.entity = 'NewBuryPort'
where tc.sdate >= '2015-01-01'
	and tc.sdate <= current_date
	and tc.workoperation not in ('Punch In', 'Punch Out')
	and tc.pressno is not null and tc.pressno <> ''	
union all
	select 'Cimarron North' 							AS businessunit,
	tc.workoperation_clean								as workoperation,
	tc.sdate,
	 (((edate::date - sdate::date) * 86400) + extract(epoch from (etime::time - stime::time)))::float as elapsed,
	tc.pressno,
	tc.footused::float,
    p.press_type,
	0::float											as footused_tabco_ex_booklet,
	0::float											as footused_tabco_good_ex_booklet
from (select *,case when workoperation ~ '^\d' then right(workoperation, (length(workoperation) - 4)) else workoperation end as  workoperation_clean 
		from dt_inovar_prod_stg.in_cimarron_fact_timecard ) tc
left join press p
	on trim(lower(p.press)) = trim(lower(tc.pressno)) and p.entity = 'Cimarron North'
left join  dt_inovar_prod_stg.in_cimarron_fact_ticket  tkt 
	on tkt.number = tc.ticket_no/* and tkt.entity = tc.entity*/
left join press pt 
	on trim(lower(pt.press)) = trim(lower(tkt.press)) and pt.entity = 'Cimarron North'
where tc.sdate >= '2023-01-01'
	and tc.sdate <= current_date
	and tc.workoperation not in ('Punch In', 'Punch Out')
	and tc.pressno is not null and tc.pressno <> ''		
union all 
	select 'Amherst Label'								AS businessunit,
	tc.workoperation_clean								as workoperation,
	tc.sdate,
	case
		when tc.edate::text = '' then EXTRACT(EPOCH FROM tc.elapsed::time)
		else (((edate::date - sdate::date) * 86400) + extract(epoch from (etime::time - stime::time)))
	end													as elapsed,
	tc.pressno,
	tc.footused,
	...
	0::float											as footused_tabco_good_ex_booklet
from (select *, /*case when entity = 'Cimarron North' and workoperation ~ '^\d' then right(workoperation, (length(workoperation) - 4)) else workoperation end*/ workoperation as  workoperation_clean 
		from dt_inovar_prod_stg.in_amherst_fact_timecard ) tc
left join press p
	on trim(lower(p.press)) = trim(lower(tc.pressno)) and p.entity = 'Amherst'
left join  dt_inovar_prod_stg.in_amherst_fact_ticket  tkt 
	on tkt.number = tc.ticket_no /*and tkt.entity = tc.entity*/
left join press pt 
	on trim(lower(pt.press)) = trim(lower(tkt.press)) and pt.entity = 'Amherst'
where tc.sdate >= '2015-01-01'
	and tc.sdate <= current_date
	and tc.workoperation not in ('Punch In', 'Punch Out')
	and tc.pressno is not null and tc.pressno <> ''	
union all 
	select  'Westfield'									AS businessunit,
	tc.workoperation_clean								as workoperation,
	tc.sdate,
	case
		when tc.edate = '' then EXTRACT(EPOCH FROM tc.elapsed::time)
	...
	0::float											as footused_tabco_good_ex_booklet
from (select *,/*case when entity = 'Cimarron North' and workoperation ~ '^\d' then right(workoperation, (length(workoperation) - 4)) else workoperation end*/ workoperation as  workoperation_clean 
		from dt_inovar_prod_stg.in_westfield_fact_timecard ) tc
left join press p
	on trim(lower(p.press)) = trim(lower(tc.pressno)) and p.entity = 'Westfield'
left join  dt_inovar_prod_stg.in_westfield_fact_ticket  tkt 
	on tkt.number = tc.ticket_no /*and tkt.entity = tc.entity*/
left join press pt 
	on trim(lower(pt.press)) = trim(lower(tkt.press)) and pt.entity = 'Westfield'
where tc.sdate >= '2015-01-01'
	and tc.sdate <= current_date
	and tc.workoperation not in ('Punch In', 'Punch Out')
	and tc.pressno is not null and tc.pressno <> ''	
union all 
	select  'Sioux Falls - LA'							AS businessunit,
	tc.workoperation_clean								as workoperation,
	tc.sdate,
	case
		when tc.edate::text  = '' then EXTRACT(EPOCH FROM tc.elapsed::time)
	...
	0::float											as footused_tabco_good_ex_booklet
from (select *,/*case when entity = 'Cimarron North' and workoperation ~ '^\d' then right(workoperation, (length(workoperation) - 4)) else workoperation end*/ workoperation as  workoperation_clean 
		from dt_inovar_prod_stg.in_siouxfalls_la_fact_timecard) tc
left join press p
	on trim(lower(p.press)) = trim(lower(tc.pressno)) and p.entity = 'Sioux Falls - LA'
left join  dt_inovar_prod_stg.in_siouxfalls_la_fact_ticket  tkt 
	on tkt.number = tc.ticket_no /*and tkt.entity = tc.entity*/
left join press pt 
	on trim(lower(pt.press)) = trim(lower(tkt.press)) and pt.entity = 'Sioux Falls - LA'
where tc.sdate >= '2015-01-01'
	and tc.sdate <= current_date
	and tc.workoperation not in ('Punch In', 'Punch Out')
	and tc.pressno is not null and tc.pressno <> ''
        ...
        SELECT DISTINCT 'Westfield' AS location,
          ...
          WHERE ibfg.financialstatementclass::text = 'Sales'::text
          	AND NOT (gld.invoice_id IN ( SELECT gld_1.invoice_id
                  ...
                  WHERE ibfg_1.financialstatementclass::text = 'Discounts, returns and allowances'::text))
 	UNION ALL
        SELECT DISTINCT 'Sioux Falls - LA' AS location,
            gld.invoice_id::integer,
            ibfg.financialstatementclass
           FROM dt_inovar_prod_stg.in_siouxfalls_la_fact_invoice_gl_distribution gld
             LEFT JOIN dt_inovar_prod_stg.in_siouxfalls_la_fact_glchartofaccounts ibfg ON gld.account_number::text = ibfg.accountnumber::text
          WHERE ibfg.financialstatementclass::text = 'Sales'::text
          	AND NOT (gld.invoice_id IN ( SELECT gld_1.invoice_id
                   FROM dt_inovar_prod_stg.in_siouxfalls_la_fact_invoice_gl_distribution gld_1
                     LEFT JOIN dt_inovar_prod_stg.in_siouxfalls_la_fact_glchartofaccounts ibfg_1 ON gld_1.account_number::text = ibfg_1.accountnumber::text
                  WHERE ibfg_1.financialstatementclass::text = 'Discounts, returns and allowances'::text)) 
        ), invoices AS (
         SELECT 
...
--             LEFT JOIN packingslip_info ps ON inv.location = ps.location AND inv.packslipitem_id::text = ps.packingslip_item_id::text  
	...
	UNION all
         SELECT 
            ...
            NULL::text 															AS ticketitemid
        FROM dt_inovar_prod_stg.in_clearwater_fact_sage_ar_invoicehistoryheader sih
            LEFT JOIN dt_inovar_prod_stg.in_clearwater_fact_sage_ar_invoicehistorydetail sid ON sih.invoiceno::text = sid.invoiceno::text
            																		and sih.headerseqno::text = sid.headerseqno::text
            LEFT JOIN dt_inovar_prod_stg.in_clearwater_fact_sage_ar_customer cus ON sih.customerno::text = cus.customerno::text
            LEFT JOIN dt_inovar_prod_stg.in_clearwater_fact_sage_ar_salesperson spn ON sih.salespersonno::text = spn.salespersonno::text
            LEFT JOIN dt_inovar_prod_stg.in_clearwater_fact_sage_so_salesorderhistoryheader soh ON sih.salesorderno::text = soh.salesorderno::text
...
--            	ON sih.salesorderno::text = sohd.salesorderno::text
            /*left*/ join dt_inovar_prod_stg.in_clearwater_fact_sage_gl_account sg on sg.accountkey = sid.salesacctkey
            ...
            left join dt_inovar_prod_stg.in_clearwater_fact_sage_ar_salesperson sal on sal.salespersonno = cus.salespersonno
	...
	UNION ALL
        ...
        FROM dt_inovar_prod_stg.in_clearwater_fact_sage_ar_invoicehistoryheader sih
            ...
            join (select distinct ihd.invoiceno, ihd.headerseqno
					FROM dt_inovar_prod_stg.in_clearwater_fact_sage_ar_invoicehistorydetail ihd
					JOIN dt_inovar_prod_stg.in_clearwater_fact_sage_ar_invoicehistoryheader ihh ON ihh.InvoiceNo = ihd.InvoiceNo AND ihh.HeaderSeqNo = ihd.HeaderSeqNo
					JOIN dt_inovar_prod_stg.in_clearwater_fact_sage_gl_account gl ON gl.AccountKey = ihd.SalesAcctKey) gl
	...
	select 'Westfield' as businessunit, number, concat('DL-', number) as company_customer_number, company, otsname, sales_rep_no, itsname, cust_serv_no 
		...
		union all
	select 'Sioux Falls - LA' as businessunit, number, concat('SF-', number) as company_customer_number, company, otsname, sales_rep_no, itsname, cust_serv_no 
			from dt_inovar_prod_stg.in_siouxfalls_la_dim_customer
		union all	
	...
	from dt_inovar_prod_stg.in_clearwater_precision_dim_customers cus
	left join dt_inovar_prod_stg.in_clearwater_fact_sage_ar_salesperson sal on sal.salespersonno = cus.salespersonno
	),
	final_pure_sales AS (
	SELECT 
...
all_tickets AS (
  ...
  union all
    ...
    UNION ALL
      ...
      UNION ALL
         SELECT 
         	'Sioux Falls'::text 						AS businessunit,
            concat('SF-', tkt.customernum) 				AS company_customer_number,
            concat('SF-', tkt.number) 					AS company_ticket_number,
            tkt.orderdate::text,
            COALESCE(cus.company, tkt.customername) 	AS customername,
            tkt.otsname,
            tickettype::int,
            ...
            FROM dt_inovar_prod_stg.in_siouxfalls_la_fact_ticket tkt
             LEFT JOIN dt_inovar_prod_stg.in_siouxfalls_la_dim_customer cus ON tkt.customernum::text = cus.number::text
             LEFT JOIN dt_inovar_prod_stg.in_siouxfalls_la_fact_ticketitem tktit ON tkt.number::text = tktit.ticketnumber::text
             left join dt_inovar_prod_edw.u_mat_backlog_type_test_bookings bv on
             			bv.number::text = tkt.number::text AND bv.ticketitemid = tktit.id::TEXT AND bv."location" = 'Westfield'
          WHERE tkt.orderdate::date >= '2023-01-01'::date
          GROUP BY 1,2,3,4,5,6,7
   ...
   UNION ALL
	...
	on cus.customer_number = tkt_cte.customer_number and cus.rank = 1
        ...
        UNION ALL
         SELECT 
           ...
           FROM dt_inovar_prod_stg.in_clearwater_fact_sage_so_salesorderhistoryheader tkt
             LEFT JOIN ( SELECT 
                    salesorderno,
                    lastextensionamt
                   FROM dt_inovar_prod_stg.in_clearwater_fact_sage_so_salesorderhistorydetail
                  WHERE cancelledline::text = 'N'::text AND deleteddate IS NULL) tktit 
                  		ON tkt.salesorderno::text = tktit.salesorderno::text
             LEFT JOIN dt_inovar_prod_stg.in_clearwater_fact_sage_ar_salesperson spn ON spn.salespersonno::text = tkt.salespersonno::text
        ...
        UNION ALL
         SELECT 
           ...
           FROM dt_inovar_prod_stg.in_clearwater_fact_sage_so_salesorderheader tkt
             LEFT JOIN dt_inovar_prod_stg.in_clearwater_fact_sage_so_salesorderdetail tktit ON tkt.salesorderno::text = tktit.salesorderno::text
             LEFT JOIN dt_inovar_prod_stg.in_clearwater_fact_sage_ar_salesperson spn ON spn.salespersonno::text = tkt.salespersonno::text
          WHERE tkt.orderdate >= '2023-01-01'::date AND NOT (tkt.salesorderno::text IN ( SELECT DISTINCT in_clearwater_fact_sage_so_salesorderhistoryheader.salesorderno
                   FROM dt_inovar_prod_stg.in_clearwater_fact_sage_so_salesorderhistoryheader))
          GROUP BY 1,2,3,4,5,6,7
          ),  
    final_data AS (
         SELECT 
           ...
           FROM all_tickets
           /*left join (
           		select company_ticket_number, min(booking_type) as booking_type
				from dt_inovar_prod_edw.u_mat_bookings_to_invoice_sandbox
				group by 1) book 
           		on  book.company_ticket_number = all_tickets.company_ticket_number*/
     		WHERE 
    ...
    ticket_sandbox AS (
        SELECT 
            ...
            tkt.linetotal,
           	case
           		when tkt.businessunit in ('Ft. Lauderdale', 'Dallas', 'Newburyport', 'Milwaukee', 'Westfield', 'Cimarron North', 'Amherst Label')
           			then book.booking_type
       			when tkt.businessunit in ('Kansas City') and customername ILIKE '%Tabco, Inc.%'
       				then 'Stock Booking'
   				when tkt.businessunit in ('Kansas City')
       				then 'Custom Booking'
   				when tkt.businessunit in ('Oceanside')
					then 'Custom Booking'
				else 'error'
           	end								  			as booking_type
           	FROM final_data tkt
				...
				from dt_inovar_prod_edw.u_mat_bookings_to_invoice_sandbox
				group by 1) book 
           		on  book.company_ticket_number = tkt.company_ticket_number                  
            	)
	SELECT 
		businessunit, 
...
union all
	select 
		...
		null::float						as budget,
		null::text 						as stock_ticket_flag,
		coalesce(upper(trim(workoperation)), 'Unmapped') 						as workoperation,
		sdate,
		elapsed,
		case 
			when trim(lower(pressno)) = 'test' then '301'
			when trim(lower(pressno)) = '9c-blue' then '9C-Blue'
			else pressno
		end								as pressno,
		footused,
		case 
...
select 
		...
		pressno,
		footused,
		COALESCE(trim(press_type), 'Other Equipment (E.G., Finishing and QC)')	as press_type,
		footused_tabco_ex_booklet,
		footused_tabco_good_ex_booklet,
		case
	    	when trim(booking_type) is null then 'NA'
	    	else coalesce(trim(booking_type),'NA')
	    end																		as	booking_type,
		case 
