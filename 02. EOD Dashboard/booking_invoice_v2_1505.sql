...
create table dt_inovar_dev_stg.u_mat_6_invoice_sandbox_temp as
select * from dt_inovar_dev_stg.v_6_invoice_sandbox_temp

as select * from dt_inovar_dev_stg.v_6_invoice_sandbox_temp


drop view dt_inovar_dev_stg.v_invoice_sandbox_temp

drop table dt_inovar_dev_stg.u_mat_6_invoice_sandbox_temp

drop table dt_inovar_dev_stg.u_mat_invoice_sandbox_temp

select * from dt_inovar_dev_stg.v_invoice_sandbox_temp

CREATE OR REPLACE VIEW dt_inovar_dev_stg.v_6_invoice_sandbox_temp as
...
--            	ON sih.salesorderno::text = sohd.salesorderno::text
	...
	select
		...
		inv.invoice_date,
	    case
	    	when inv.invoice_customer_name = '' then 'Unmapped'
	    	else coalesce(inv.invoice_customer_name,'Unmapped')
	    end																						as customername,
...
booking as (
	...
	--			end as date_promise,
			...
			where replace(date_order,'None','1900-01-01') >= '2021-01-01'
	), 
	all_tickets AS (
	 SELECT
          ...
          WHERE tkt.orderdate::date >= '2021-01-01'::date
          GROUP BY 1,2,3,4,5  
      	UNION ALL
         SELECT 
    ...
    UNION ALL
        ...
        UNION ALL
         SELECT 
          ...
          WHERE tkt.orderdate::date >= '2021-01-01'::date AND tkt.orderstatus::text <> 'X'::TEXT
          GROUP BY 1,2,3,4,5
        UNION ALL
         SELECT 
            'Precision Label'::text AS businessunit,
           ...
           FROM dt_inovar_prod_stg.in_fact_sage_so_salesorderheader tkt
             ...
             LEFT JOIN dt_inovar_prod_stg.in_fact_sage_ar_salesperson spn ON spn.salespersonno::text = tkt.salespersonno::text
          WHERE tkt.orderdate >= '2021-01-01'::date AND NOT (tkt.salesorderno::text IN ( SELECT DISTINCT in_fact_sage_so_salesorderhistoryheader.salesorderno
...
backlogs as (
	...
	where snapshot_date::date >='2021-01-01'
...
-- Invoice Data
 ...
 union all
 	select 
 		final_working_days.dt,
		final_working_days.businessunit,
		backlogs.record_type,
		null::date 						as invoice_date,
		null::date 						as order_date,
		backlogs.snapshot_date::date,
		company_customer_number,
		backlogs.customername,
		null::date 						as ship_by_date,		
