...
---------------for limited columns-----------
select businessunit ,sum(invoice_revenue::float) invoice_revenue from  dt_inovar_dev_stg.u_mat_6_invoice_sandbox_temp
where invoice_date::date>'2023-01-01' and invoice_date::date<='2024-04-30'
group by 1


--------view directly from code
select businessunit ,sum(invoice_item_total::float) invoice_revenue
from dt_inovar_dev_stg.u_mat_invoice_sandbox_temp /*->>> not deleted yet*/
where invoice_date::date>'2023-01-01' and invoice_date::date<='2024-04-30'
...
where invoice_date::date>'2023-01-01' and invoice_date::date<='2024-04-30'
and ( invoice_item_id not in ('Discounts_Pure','Accounts Receivable – trade','Unexpected','Sales_Mix','Discounts_Mix')
or  invoice_item_id is null) and (invoice_type <> 'CM' or invoice_type is null)
...
select businessunit ,sum(invoice_item_total::float) invoice_revenue from
dt_inovar_prod_edw.u_mat_invoice_sandbox
where invoice_date::date>'2023-01-01' and invoice_date::date<='2024-04-30'
group by 1

/*
create table dt_inovar_dev_stg.u_mat_6_invoice_sandbox_temp
...
drop view dt_inovar_dev_stg.v_6_invoice_sandbox_temp */
