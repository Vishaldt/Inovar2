...
select 'edw',date_trunc('month',sdate::date)::date dt ,count(*), sum(footused)footused, sum(elapsed)elapsed 
from dt_inovar_prod_edw.u_mat_sales_bookings_eod_dashboard
where sdate>'2024-01-01'
group by 1,2


select distinct businessunit,sum(elapsed)elapsed,sum(footused)footused  from dt_inovar_prod_edw.u_mat_sales_bookings_eod_dashboard
where is_operations=1
--and workoperation ='RUN'
group by 1
...
left join press p
	on trim(lower(p.press)) = trim(lower(tc.pressno)) and p.entity = 'Ft. Lauderdale'
left join  dt_inovar_prod_stg.in_davie_fact_ticket  tkt 
	on tkt.number = tc.ticket_no /*and tkt.entity = tc.entity*/
left join press pt 
	on trim(lower(pt.press)) = trim(lower(tkt.press)) and pt.entity = 'Ft. Lauderdale'
where tc.sdate >= '2015-01-01'
	and tc.sdate <= current_date
	and tc.workoperation not in ('Punch In', 'Punch Out')
	and tc.pressno is not null and tc.pressno <> ''
      ...
      UNION ALL
        ...
        UNION ALL
         SELECT
            ...
            (colorchangecost::double precision * nocolorchanges::double precision) AS linetotal
           FROM dt_inovar_prod_stg.in_westfield_fact_ticket tkt
             LEFT JOIN dt_inovar_prod_stg.in_westfield_dim_customer cus ON tkt.customernum::text = cus.number::text          WHERE tkt.orderdate::date >= '2023-01-01'::date
        UNION ALL
         SELECT 
...
select 
	...
	case 
			when trim(lower(inv.press_type)) = 'flexo' 						then 'Flexo Press'
			when trim(lower(inv.press_type)) = 'digital' 					then 'Digital Press'
			when trim(lower(inv.press_type)) = 'hybrid' 					then 'Hybrid Press'
			when trim(lower(inv.press_type)) = 'digital finishing' 			then 'Digital Finishing Equipment'
			when trim(lower(inv.press_type)) = 'large format' 				then 'Large Format Press'
			when trim(lower(inv.press_type)) = 'other' 						then 'Other Equipment (E.G., Finishing and QC)'
			when trim(lower(inv.press_type)) = 'flexo finishing equipment'	then 'Other Equipment (E.G., Finishing and QC)'
			when trim(lower(inv.press_type)) = 'rewinder'					then 'Other Equipment (E.G., Finishing and QC)'
			when trim(lower(inv.press_type)) = 'slitter/rewinder'			then 'Other Equipment (E.G., Finishing and QC)'
			else COALESCE(initcap(trim(inv.press_type)), 'Other Equipment (E.G., Finishing and QC)')
		end										as press_type,