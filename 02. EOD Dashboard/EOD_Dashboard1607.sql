...
-- All commented code removed on 16-07
...
--
--	
--INSERT INTO dt_inovar_prod_edw.u_mat_sales_bookings_eod_dashboard
--SELECT *  FROM dt_inovar_prod_edw.v_sales_bookings_eod_dashboard	
--
--
--
--
----CALL dt_inovar_prod_edw.eod_dashboards()
--
--		
--CREATE TABLE dt_inovar_prod_edw.u_mat_sales_bookings_eod_dashboard as 
--SELECT * FROM dt_inovar_prod_edw.v_sales_bookings_eod_dashboard
-- 
--drop view dt_inovar_prod_edw.v_sales_bookings_eod_dashboard_test
--
--
...
select 'Milwaukee' 										AS businessunit,
	...
	0::float											as footused_tabco_ex_booklet,
	0::float											as footused_tabco_good_ex_booklet 
from (select *,  workoperation as workoperation_clean 
...
invoice as(
	...
	UNION all
        ...
        FROM dt_inovar_prod_stg.in_clearwater_fact_sage_ar_invoicehistoryheader sih
            ...
            join dt_inovar_prod_stg.in_clearwater_fact_sage_gl_account sg on sg.accountkey = sid.salesacctkey
...
all_tickets AS (
  ...
  union all
    ...
    UNION ALL
      ...
      UNION ALL
         SELECT 
         	'Sioux Falls - LA'::text 					AS businessunit,
            concat('LA-', tkt.customernum) 				AS company_customer_number,
            concat('LA-', tkt.number) 					AS company_ticket_number,
            tkt.orderdate::text,
            COALESCE(cus.company, tkt.customername) 	AS customername,
            tkt.otsname,
            tickettype::int,
            ...
            FROM dt_inovar_prod_stg.in_siouxfalls_la_fact_ticket tkt
             ...
             left join dt_inovar_prod_edw.u_mat_backlog_type_test_bookings bv on
             			bv.number::text = tkt.number::text AND bv.ticketitemid = tktit.id::TEXT AND bv."location" = 'Sioux Falls - LA'
          WHERE tkt.orderdate::date >= '2023-01-01'::date
          GROUP BY 1,2,3,4,5,6,7
        ), ticket_other_rev AS (
        SELECT 
   ...
   UNION ALL
    ...
    final_data AS (
         SELECT 
            ...
            case
		    	when ((all_tickets.businessunit in ('Ft. Lauderdale', 'Dallas', 'Newburyport', 'Milwaukee', 'Westfield', 'Cimarron North', 'Amherst Label','Sioux Falls - LA') and tickettype = '0')
         ...
         SELECT
            ...
            case
		    	when ((ticket_other_rev.businessunit in ('Ft. Lauderdale', 'Dallas', 'Newburyport', 'Milwaukee', 'Westfield', 'Cimarron North', 'Amherst Label','Sioux Falls - LA') and tickettype = '0') ---
    ...
    ticket_sandbox AS (
        SELECT 
            ...
            tkt.linetotal,
           	case
           		when tkt.businessunit in ('Ft. Lauderdale', 'Dallas', 'Newburyport', 'Milwaukee', 'Westfield', 'Cimarron North', 'Amherst Label','Sioux Falls - LA')	---
