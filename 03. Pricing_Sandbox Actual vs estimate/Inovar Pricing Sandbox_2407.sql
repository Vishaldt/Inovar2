...
----------------------------------------- v2 live on (03/08)
-- Added Sioux Falls LA on 17-07
-- Added Ocean side from dt_inovar_prod_stg.in_precision_fact_r_cm_operations_to_labeltraxx on 24-07
-- Changed year for kansas and ocean side on 24-07
...
/*truncate  table  dt_inovar_prod_edw.u_mat_inovar_pricing_sandbox 

insert into dt_inovar_prod_edw.u_mat_inovar_pricing_sandbox 
select * from dt_inovar_prod_edw.v_inovar_pricing_sandbox




create table dt_inovar_prod_edw.u_mat_inovar_pricing_sandbox as
select * from dt_inovar_prod_edw.v_inovar_pricing_sandbox

select dateshipped ,* from dt_inovar_prod_edw.u_mat_inovar_pricing_sandbox


select businessunit ,count(*),min(orderdate),max(orderdate) orderdate from dt_inovar_prod_edw.u_mat_inovar_pricing_sandbox
group by 1


drop table dt_inovar_prod_edw.u_mat_inovar_pricing_sandbox
...
left join dt_inovar_prod_edw.u_mat_inovar_press_type pd
		on
	trim(lower(tkt.press)) = trim(lower(pd.press))
	and pd.businessunit = 'Sioux Falls - LA'
left join dt_inovar_prod_stg.in_siouxfalls_la_dim_equipment eq 
		on
	trim(lower(eq.number)) = trim(lower(tkt.press))
left join dt_inovar_prod_edw.v_inovar_customer_master cus 
		on
	trim(lower(cus.number)) = trim(lower(tkt.customernum))
		and cus.businessunit = 'Sioux Falls - LA'
	where
		tkt.orderdate >= '2020-01-01'
),
oceanside as (
select 
	...
	'Carlsbad'::text AS location,
    'Oceanside'::text AS company,
	...
	custponum,
	/*case 
		when turnbar = true then 'True'
		when turnbar = false then 'False'
	end as */turnbar ,
	concat('Carlsbad-', customernum) AS company_customer_number,
	customernum,
	estfootage,
	stocknum2,
	stockwidth2,
	...
	actualpresshours,
	actualtotalhours,
	actualpressrate::text,
	actualpresscost,
	actualrewindingrate::text,
	actualrewindingcost::float,
	actualfanfoldrate,
	actualfanfoldcost,
	actualpackagingrate,
